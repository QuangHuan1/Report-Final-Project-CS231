...
def trimap(image, size, erosion=False):
    ...
    if erosion is not False:
        erosion = int(erosion)
        erosion_kernel = np.ones((5,5), np.uint8)                     ## Design an odd-sized erosion kernel
        image = cv2.erode(image, erosion_kernel, iterations=1)  ## How many erosion do you expect
